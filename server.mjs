import express from "express";
import { v4 as uuidv4 } from 'uuid';
import contatos from "./data/contatos.mjs";

const app = express();

// Middleware
app.use(express.json());

// Rota raiz
app.get('/', (req, res) => {
    res.send(`<h1 style='color: blue;'>Hands-on docker: Backend</h1><p>Olá Mundo</p>`);
});

// GET /contatos - listar contatos
app.get('/contatos', (req, res) => {
    res.status(200).json({
        error: false,
        contatos
    });
});

// GET /contatos/:id - obtem 1 contato
app.get('/contatos/:id', (req, res) => {
    const id = req.params.id;
    const contato = contatos.find((contato) => contato.id == id);

    if (!contato) 
        return res.status(404).json({
            error: true,
            message: 'Contato não encontrado'
        });

    res.status(200).json({
        error: false,
        contato
    });
});

// POST /contatos - criar novo contato
app.post('/contatos', (req, res) => {
    const { nome, genero, telefone, email } = req.body;

    if (!nome || !genero || !telefone || !email)
        return res.status(400).json({
            error: true,
            message: 'Entrada inválida'
        });

    if (contatos.find((contato) => contato.email === email))
        return res.status(400).json({
            error: true,
            message: 'Email já cadastrado'
        });

    const contato = { id: uuidv4(), nome, genero, telefone, email };
    contatos.push(contato); // Adiciona ao array

    res.status(201).json({
        error: false,
        contato
    });
});

// PUT /contatos/:id - editar um contato
app.put('/contatos/:id', (req, res) => {
    const id = req.params.id;
    const contato = contatos.find((contato) => contato.id == id);

    if (!contato)
        return res.status(404).json({
            error: true,
            message: 'Contato não encontrado'
        });

    const { nome, genero, telefone, email } = req.body;

    if (email && contatos.some((c) => c.email === email && c.id !== id))
        return res.status(400).json({
            error: true,
            message: 'Email já cadastrado'
        });

    if (nome) contato.nome = nome;
    if (genero) contato.genero = genero;
    if (telefone) contato.telefone = telefone;
    if (email) contato.email = email;

    return res.status(200).json({
        error: false,
        message: 'Contato atualizado com sucesso'
    });
});

// DELETE /contatos/:id - deletar um contato
app.delete('/contatos/:id', (req, res) => {
    const id = req.params.id;
    const index = contatos.findIndex((contato) => contato.id == id);

    if (index === -1)
        return res.status(404).json({
            error: true,
            message: "Contato não encontrado"
        });

    contatos.splice(index, 1); // Remove o contato encontrado
    return res.status(200).json({
        error: false,
        message: "Contato deletado com sucesso"
    });
});

// Inicia o servidor
app.listen(3000, () => {
    console.log('Servidor iniciado na porta 3000!');
});
